...
function listHandler(mdStr, execStr) {
    ...
    for (i = 0; i < helper.length; i++) {
        if ((line = /^((\s*)((\*|\-)|\d(\.|\))) ([^\n]+))/.exec(helper[i])) !== null) {
            if (!line[2]) {
                ...
                nestStat = Math.round(line[2].replace(/\t/, '    ').length / indent);
            }
            while (status > nestStat) {
                replaceStr += helper1.pop();
                status--;
            }
            while (status < nestStat) {
                lineListType = line[4] ? ['<ul>', '</ul>'] : ['<ol>', '</ol>'];
                replaceStr += lineListType[0];
                helper1.push(lineListType[1]);
                status++;
            }
    ...
    //    casca--;
    //}
    replaceStr += listType[1];

    return mdStr.replace(execStr[0], replaceStr + '\n');
}
//handle list line
//function parseListLine(line) {
//    var nestStat, indent, replaceStr, casca, lineListType, helper1;
//
//    if (!line[2]) {
//        nestStat = 0;
//    } else {
